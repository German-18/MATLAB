function [x,y,U,iter,salida]=JacobiEDP(funseg,funa,funb,func,fund,A,B,C,D,tol,maxit,Nx,Ny)
...
while  (iter < maxit)  && (error>tol)
    ...
    for ii=2:Nx+1
        for jj=2:Ny+1
            U(ii,jj) = (alfacuad*(Uold(ii+1,jj)+Uold(ii-1,jj))+...
                Uold(ii,jj+1)+Uold(ii,jj-1)-hy2*funseg(x(ii),y(jj)))/beta;
        end
    end
